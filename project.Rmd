...
For our 4th model, we try a lasso model to see if it can penalize unimportant variables and get a better accuracy and sensitivity.
...
This model, using the best lambda (of 0.001232847) had an accuracy rate of 89.31%, which is noticeably higher than the No Information Rate. Most importantly for our model, the sensitivity is 47.9%, meaning, for all the people that truly attrite, we predict correctly 47.9% of the time. Ideally, we would want something higher still, but this sensitivity is much better than the sensitivities in our previous models.
...
confusionMatrix(data = predict(attrition_lasso_best, type = "raw"),
                ...
                positive = "Yes") 
```
This model, using the best lambda (of 0.001232847) had an accuracy rate of 89.6%.  The sensitivity also went up to 49.7%.  The specificity is at 97.4%, meaning that we predict the people who will remain in their positions almost perfectly.



Now we can examine which coefficiants aren't helpful and also look at which coefficiants are the most important. Do you see a difference from the previous importance plot? ((Do we need this?  If so, we might wanna make a 6th model, and that's kind of a lot of models...))
```{r}
...
I agree, we can delete this - Colleen
```{r}
set.seed(253)
...
Model | Accuracy | Specificity | CV acc 
---------------------- | ------------- | ------------- | ------------- 
**Model 1: Logistic Regression with Exploratory Variables** | 0.8484 | 0.98721 | 0.8416292 
**Model 2: Logistic Regression with All Variables** | 0.897 | 0.8785603 | 0.8785603 
**Model 3: Logistic Regression with Most Important Variables** | 0.8397 | 0.8416008 | 0.8416008 
**Model 4: LASSO with "Best" Lambda** | 0.8931 | 0.97442 | 0.882
**Model 5: LASSO with Different Lambda** | 0.896 | 0.8416008 | 0.8416008


ROUGH DRAFT OF CONCLUSION
To pick the final model that we want to use, we need to remember what the goal was of this model-building process: we want to predict who is most likely to attrite.  That is, who is most likely to leave their position and not be replaced?  The sensitivity will tell us that.  The model with the highest sensitivity is ____, and therefore, is the best model.

However, in the future, we should keep trying to find a model with an even higher sensitivity, because the sensitivity is not "good" in any of these models, and we don't want to be making many inaccurate predictions about such an important issue.
