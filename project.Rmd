...
#Those who worked overtime have 4.15 times the odds of attriting than those who don't work overtime. 
```
...
#sensitivity is .13
#therefore, this model does not seem very good
...
```{r}
#now we try a lasso model, to see if it can penalize unimportant varibles and get the accuracy and sensitivity to be better
set.seed(253)
...
```
```{r}
#all vars lasso model with best lambda... but we don't rlly need this because it automatically chose the best lambda out of the range of lambdas in the model above... however, does just putting one value of lambda make
